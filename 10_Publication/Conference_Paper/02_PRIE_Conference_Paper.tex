...
\begin{IEEEkeywords}
Placement Readiness, Educational Data Mining, Explainable AI, Student Profile Vector, Platt Calibration, Algorithmic Recourse, Multimodal Fusion, Curricular Knowledge Graph.
\end{IEEEkeywords}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/SPV_Pipeline.png}
\caption{End-to-end architectural pipeline of the Placement Readiness Intelligence Engine (PRIE). Telemetry streams from academic records, coding sandboxes, 2D spatial resume documents, and multimodal mock interviews are normalized into the 22-dimensional Student Profile Vector ($x_{\text{spv}}$). Calibrated predictive modeling drives TreeSHAP feature attribution, constrained DiCE prescriptive recourse, Kahn topological curricular scheduling, and RAG guardrail retrieval.}
\label{fig_arch}
\end{figure*}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.90\columnwidth]{figures/fig1_calibration_reliability.png}
\caption{Reliability diagram on hold-out test evaluation. Platt scaling contracts Expected Calibration Error from $0.0370$ to $0.0212$ (Seed 42 holdout) and from $0.0570 \pm 0.0082$ to $0.0350 \pm 0.0057$ across the 5-seed battery, aligning confidence with empirical accuracy.}
\label{fig_calib}
\end{figure}
